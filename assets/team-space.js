(function () {
  ...
  function breedingAnalysis(rows) {
    ...
    const key = r => breedCompare === 'species' ? r.speciesName || '未记录品种' : breedCompare === 'year' ? r.date.slice(0, 4) || '未记录年份' : r.batchId ? 'batch:' + r.batchId : !r.motherId || ['manual', '__keep__'].includes(r.motherId) ? 'name:' + r.motherName : r.motherId;
  ...
  function breedingFilters(rows) {
    ...
    return `<div class="ts-breeding-filters">${select('year', '年份', rows.map(r => r.date.slice(0, 4)), breedYear)}${select('species', '品种', rows.map(r => r.speciesName || '未记录品种'), breedSpecies)}${select('mother', '单龟 / 批次', rows.map(r => r.motherName), breedMother)}</div>`;
  }
  ...
  function bind() {
    ...
    root.querySelectorAll('[data-team-archive]').forEach(select => {
      select.dataset.archiveDirectory = '';
      select.__directorySource = { turtles: select.dataset.teamArchive === 'breeding' ? team.breedingParents || [] : team.ledgerTurtles || [], pools: team.breedingPools || team.pools || [] };
    });
    ...
    root.querySelector('#ts-form')?.addEventListener('submit', async e => {
      ...
      if (dialog === 'breeding') {
        values.batchId = values.motherId?.startsWith('batch:') ? values.motherId.slice(6) : '';
        if (values.batchId) values.motherId = '';
    ...
    host.bindArchivePicker?.();
    previewPermissions();
    root.onkeydown = e => {
      if (e.key === 'Escape' && dialog && !busy) { dialog = ''; repaint(); }
      if (e.key === 'Tab' && dialog) {
        const nodes = [...root.querySelectorAll('.ts-dialog button:not(:disabled), .ts-dialog input, .ts-dialog select')].filter(node => !node.disabled && node.getClientRects().length);
