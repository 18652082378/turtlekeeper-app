(function () {
  ...
  function overview() {
    const r = team.report;
    const due = team.tasks.filter(t => !t.done && t.due <= today());
    return `<section class="ts-workhero"><span class="ts-kicker">${E(month)} · 团队概览</span><h2>每一笔记录，<br>都是经营的底气。</h2><p>${due.length ? `有 ${due.length} 项到期任务，记得安排照料。` : '把今天的照料，变成看得见的进度。'}</p>${can('tasks') ? button('tab', '查看今日任务 ' + icon('arrow'), 'data-tab="tasks"') : ''}<div class="ts-workmark">${icon('chart')}</div></section>
    <div class="ts-metrics">${metric('在养档案', can('dashboard') ? team.turtles.filter(t => !['已死亡', '已转让'].includes(t.status)).length : '—', '主账号云端档案')}${metric('协作成员', `${team.members.filter(m => m.status === 'active' || !m.status).length}<em>/ 6</em>`, '另含 1 位主账号')}${r ? metric(reportRange.mode === 'month' ? '所选月份销售收入' : '所选期间销售收入', money(r.income), '元 · 已记录销售', 'ts-positive') + metric(reportRange.mode === 'month' ? '所选月份收支结余' : '所选期间收支结余', money(r.balance), '元 · 收入减现金支出') : ''}</div>
    ${attentionPanel()}
  ...
  function breedingView(hatching = false) {
    ...
    return `<section class="ts-breed-head"><div><span class="ts-kicker">从一窝蛋，到新的生命</span><h2>${hatching ? '孵化管理' : '繁殖记录'}</h2><p class="ts-muted">${hatching ? '按窝记录分批出壳，幼龟档案自动进入主账号看板。' : '与主账号个人空间共用记录，团队一起跟进产蛋与受精。'}</p></div>${canEdit('breeding') ? button('breeding', icon('plus') + '新增繁殖', '', 'ts-primary') : '<span class="ts-tag">只读</span>'}</section>${breedingFilters(allNests)}${breedingAnalysis(rows)}
    <section class="ts-panel"><div class="ts-section-title"><h2>${hatching ? '按窝跟进' : '窝次明细'} <span class="ts-count">${rows.length}</span></h2></div><input class="ts-search" type="search" data-filter="breeding" value="${E(filter)}" placeholder="搜索种母、日期或养殖池" aria-label="搜索繁殖记录"><div class="ts-chips">${[['all', '全部'], ['open', '进行中'], ['closed', '已结束']].map(([v, l]) => button('breeding.filter', l, `data-value="${v}"`, breedingStatus === v ? 'selected' : '')).join('')}</div>
    <div class="ts-nests">${rows.map(r => `<article class="ts-nest"><div class="ts-nest-title"><div><small>${E(r.date)}${r.poolName ? ' · ' + E(r.poolName) : ''}</small><h3>${E(r.motherName)}</h3></div><span class="ts-tag">${r.incubationClosed ? '已结束' : '进行中'}</span></div>
    ${!hatching && safePhoto(r.photo) ? `<img class="ts-nest-photo" src="${E(safePhoto(r.photo))}" alt="${E(r.motherName)} 繁殖照片" loading="lazy">` : ''}
    ...
    ${!hatching && r.note ? `<p class="ts-nest-note">${E(r.note)}</p>` : ''}
    ${hatching ? `<details class="ts-hatch-events" ${r.hatchEvents.length ? 'open' : ''}><summary>分批出壳记录 · ${r.hatchEvents.length} 次</summary>${[...r.hatchEvents].reverse().map(e => `<div><span>${E(e.date)}<small>${e.historicalLink ? '历史档案补建 · 不重复计数' : '独立出壳批次'}</small></span><b>${e.count} 只</b></div>`).join('') || '<p>还没有分批出壳记录。</p>'}</details>` : ''}
  ...
  const breeding = () => breedingView(false), hatching = () => breedingView(true);
  ...
  function workspace() {
    const tabs = [['overview', 'grid', '概览'], ['ledger', 'book', '账本'], ['reports', 'chart', '报表'], ['breeding', 'sprout', '繁殖'], ['hatching', 'egg', '孵化'], ['care', 'check', '护理'], ['tasks', 'check', '任务'], ['members', 'users', '成员'], ['logs', 'clock', '记录'], ['approvals', 'shield', '审批'], ['settings', 'card', '设置']];
    ...
    ${!team.active ? `<section class="ts-panel">${empty('团队会员已到期', '成员协作已暂停，主账号个人数据仍然保留。')}${team.own ? button('membership', '查看会员方案', '', 'ts-primary ts-wide') : ''}</section>` : ({ overview, ledger, reports, breeding, hatching, care, tasks, members, logs, approvals, settings })[tab]()}`;
  }
