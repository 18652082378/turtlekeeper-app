(function (root) {
  ...
  const recordFields = ['turtles', 'ledgerRecords', 'memos', 'careRecords', 'careCustomItems', 'carePlans', 'breedingRecords', 'turtlePools', 'activityLogs', 'customSpecies', 'satisfactionReviews', 'feedbackItems'];
  const setFields = ['keptSpecies', 'marketFavoriteIds', 'marketHistoryIds'];
  const fieldNames = { turtles: '档案', ledgerRecords: '账本', memos: '提醒', careRecords: '养护记录', careCustomItems: '养护事项', carePlans: '喂食方案', breedingRecords: '繁殖', turtlePools: '龟池', activityLogs: '操作记录', customSpecies: '品种', accountName: '昵称', accountAvatar: '头像', themeColor: '主题', professionalOutput: '报表设置', satisfactionRating: '评价' };
      ...
      for (const [field, value] of Object.entries(snapshot.data || {})) {
        if (recordFields.includes(field) && Array.isArray(value)) {
          ...
          value.forEach((record, index) => {
            ...
            if (['activityLogs', 'careRecords', 'carePlans'].includes(field)) return;
