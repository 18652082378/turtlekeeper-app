...
async function main() {
  ...
  owner.data.turtles.push({id:'batch-parent',batchId:'parent-batch',batchName:'整批产蛋组',speciesCode:'GHG',speciesName:'果核蛋���',status:'正常饲养'});
  await edit('breeding', {...fields,motherId:'',motherName:'',batchId:'parent-batch'},2);
  const batchNest = owner.data.breedingRecords[0];
  assert.equal(batchNest.motherId,''); assert.equal(batchNest.batchId,'parent-batch');
  assert.equal(batchNest.motherName,'整批产蛋组'); assert.equal(batchNest.speciesName,'果核蛋龟');
  assert.equal((await get(2)).breedingRecords.find(r=>r.id===batchNest.id).batchId,'parent-batch');
  await assert.rejects(edit('breeding',{...fields,batchId:'missing-batch'},2), /有效批次/);
  await edit('hatch',{...hatch,id:batchNest.id,count:1,speciesCode:'GHG',eventId:'batch-nest-hatch-event-01'},2);
  assert.equal(owner.data.breedingRecords.find(r=>r.id===batchNest.id).hatchCount,1);
  owner.data.turtles.find(t=>t.id==='batch-parent').status='已转让';
  await edit('breeding',{...fields,id:batchNest.id,motherId:'',motherName:'',batchId:'parent-batch'},2);
  assert.equal(owner.data.breedingRecords.find(r=>r.id===batchNest.id).batchId,'parent-batch','historical batches remain editable');
  await edit('breeding',{...fields,id:batchNest.id,batchId:'',motherId:'manual',motherName:'改为手动记录'},2);
  assert.equal(owner.data.breedingRecords.find(r=>r.id===batchNest.id).batchId,'');
