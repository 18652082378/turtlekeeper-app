...
(async () => {
  ...
  try {
    ...
    await post('/api/analytics/visit', { ...visit, event: 'end' });
    const analyticsBefore = JSON.parse(await fs.readFile(durable, 'utf8')).appAnalytics;
    const sessionBefore = Object.values(Object.values(analyticsBefore.days)[0].sessions)[0];
    assert.equal(sessionBefore.activeModule, '看板');
    assert.ok(sessionBefore.endedAt);
